...
func (x *Product) fastReadField6(buf []byte, _type int8) (offset int, err error) {
	...
	x.CategoryName = append(x.CategoryName, v)
	return offset, err
}
...
func (x *Product) fastWriteField6(buf []byte) (offset int) {
	if len(x.CategoryName) == 0 {
		return offset
	}
	for i := range x.GetCategoryName() {
		offset += fastpb.WriteString(buf[offset:], 6, x.GetCategoryName()[i])
	}
	return offset
}
...
func (x *Product) sizeField6() (n int) {
	...
	for i := range x.GetCategoryName() {
		n += fastpb.SizeString(6, x.GetCategoryName()[i])
	}
	return n
}
